...
class GameGrid(object):
    ...
    def get_pos(self, x, y):
        x0 = x * (size + border) + border + offset
        y0 = y * (size + border) + border + offset
        x1 = x0 + size
        y1 = y0 + size
        return x0, y0, x1, y1
    ...
    def draw_line(self, used):
        for i in range(len(used) - 1):
            ...
            w.create_line((x0 + x1) / 2, (y0 + y1) / 2, (x2 + x3) / 2, (y2 + y3) / 2, fill="red", width=2)
            w.after(300, self.erase_line, used)
